<template>
  <div>
    <el-table
      ...
      <el-table-column
        ...
        <el-tooltip class="item" effect="dark" content="发布" placement="top-start">
          <el-button type="primary" @click="dialogVisible2 = true" icon="el-icon-circle-check"></el-button>
        </el-tooltip>
    ...
    <el-button type="primary" @click="dialogVisible2 = false;go()">确 定</el-button>
  </span>
    </el-dialog>
...
export default {
  ...
  methods:{
    findAllDraft(){
      instance.post("/article/findAllByUser",{'createBy':this.id,'status':1}).then(res=>{
    ...
    go(){
      instance.post("/article/release",{'id':this.article.id}).then(res=>{
        if (res.data.code == 200){
          this.$message.success(res.data.msg);
          this.findAllDraft()
          return;
